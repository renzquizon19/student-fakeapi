        ...
        success: function (response) {//once the request successfully process to the server side it will return result here
            ...
            if(response.length) {
	            ...
	            $.each(response, function(key,value) {
					...
					html += "<button class='btn btn-sm btn-primary mt-2' data-toggle='modal' data-target='#edit-student-modal' data-id='"+value.id+"'>Edit</button>";
...
function get() 
{
	$(document).delegate("[data-target='#edit-student-modal']", "click", function() {
		...
		$.ajax({
	        ...
	        data: {student_id:studentId}, //set data
	        beforeSend: function () {//We add this before send to disable the button once we submit it so that we prevent the multiple click
	        ...
	        success: function (response) {//once the request successfully process to the server side it will return result here
	            ...
	            $("#edit-form [name=\"birthday\"]").val(response.birthday);
                $("#edit-form [name=\"address\"]").val(response.address);
                $("#edit-form [name=\"course\"]").val(response.course);
                $("#edit-form [name=\"year\"]").val(response.year);
